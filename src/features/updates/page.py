...
def update_log_markdown(version: str) -> str:
    return f"""# 求生之路2 模组管理器
## {version} 更新日志
...
class UpdateView(ScrollArea, Ui_updateView):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setupUi(self)
        self.setObjectName("UpdateInfoInterface")
        self.update_info.setMarkdown(update_log_markdown(appConstants.VERSION))
        self.update_info.anchorClicked.connect(QDesktopServices.openUrl)


__all__ = ["UpdateView", "update_log_markdown"]

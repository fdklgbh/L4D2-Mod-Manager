    ...
    def read_dir(self, path):
        ...
        for root, _, filelist in os.walk(path, topdown=True):
            ...
            for filename in filelist:
                ...
                if len(filename) <= 1:
                    raise RuntimeError(
                        "Files without an extension are not supported: {0}".format(
        ...
        with fopen(vpk_output_path, "w+b") as f:
            ...
            if self.version == 2:
                f.write(
                    struct.pack(
                        "4I",
                        0,  # embed_chunk_length (corrected later)
...
class VPK(object):
    ...
    def get_vpkfile_instance(self, path, metadata):
        ...
        return VPKFile(
            self._make_vpkfile_path(metadata),
            filepath=path,
            fopen=self.fopen,
            **metadata,
        )
    ...
    def _make_vpkfile_path(self, metadata):
        ...
        if metadata["archive_index"] != 0x7FFF:
            path = path.replace("english", "").replace(
                "dir.", "%03d." % metadata["archive_index"]
            )
    ...
    def read_header(self):
        ...
        with self.fopen(self.vpk_path, "rb") as f:
            (self.signature, self.version, self.tree_length) = struct.unpack(
                "3I", f.read(3 * 4)
            )
            ...
            # }
                ...
                raise TypeError("Invalid header, or unsupported version")
    ...
    def calculate_checksums(self):
        ...
        return (
            tree_checksum.digest(),
            chunk_hashes_checksum.digest(),
    ...
    def read_index_iter(self):
        ...
        with self.fopen(self.vpk_path, "rb") as f:
            ...
            while True:
                if (
                    self.version > 0
                    and f.tell() > self.tree_length + self.header_length
                ):
                ...
                while True:
                    ...
                    while True:
                        ...
                        if archive_index == 0x7FFF:
                            metadata[3] = (
                                self.header_length + self.tree_length + archive_offset
                            )
...
class VPKFile(object):
    ...
    def read(self, length=-1):
        ...
        if self.offset <= self.preload_length:
            data += self.preload[
                self.offset : self.offset + length if length > -1 else None
            ]
